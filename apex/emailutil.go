...
func (email *ApexEmailInfo) ToMap(emailPriorityType EmailPriorityType) map[string]string {
	data := email.Data
	data[To_] = mergeContacts(mu.ValueStringOrDefault(data, To_, ""), email.To, emailPriorityType, Sep)
	data[Cc_] = mergeContacts(mu.ValueStringOrDefault(data, Cc_, ""), email.Cc, emailPriorityType, Sep)
	data[Bcc_] = mergeContacts(mu.ValueStringOrDefault(data, Bcc_, ""), email.Bcc, emailPriorityType, Sep)
